...
class RuntimePersistenceCoordinator:
    ...
    async def finish(self, result: Any) -> None:
        ...
        try:
            snapshot = _snapshot_from_report(getattr(result, "report", None))
            for sequence, event in enumerate(events[:1000], start=1):
                if not isinstance(event, AuditEvent):
                    continue
                step_id = _opaque_id("step-", run_id, str(sequence), event.node)
                last_step_id = step_id
                await self._store.append_step(_step_from_event(step_id, event, snapshot))
                if event.toolName and event.toolCallId:
                    ...
                    await self._store.append_tool_audit(
                        _audit_from_event(audit_id, event, snapshot)
                    )
            evidence_artifact = _evidence_artifact(run_id, getattr(result, "report", None))
...
def _step_from_event(step_id: str, event: AuditEvent, snapshot: SnapshotMetadata | None) -> AgentStepRecord:
...
def _audit_from_event(audit_id: str, event: AuditEvent, snapshot: SnapshotMetadata | None) -> ToolAuditRecord:
...
def _evidence_artifact(run_id: str, report: Any) -> AgentArtifactRecord | None:
    """Persist only a typed evidence artifact's digest and snapshot binding."""

    if not isinstance(report, EvidenceReviewReport):
        return None
    try:
        ...
        return AgentArtifactRecord(
            ...
            dataSnapshotId=report.analysisSnapshotIds[0],
