"""Build a fresh model-origin collection pool with non-overlapping case IDs."""
...
EXTRAS = [
    ("analyze_projection", "cross_project_validate"),
    ("retrieve_evidence", "adjust_confounders"),
    ("cross_project_validate", "stratified_analysis"),
    ("cross_disease_validate", "adjust_confounders"),
    ("analyze_projection", "stratified_analysis"),
    ...
    ("adjust_confounders", "cross_project_validate", "retrieve_evidence"),
    ("analyze_projection", "cross_project_validate", "stratified_analysis"),
]
...
def build() -> tuple[dict, dict]:
    ...
    for family, base in BASES.items():
        for extra in EXTRAS:
            ...
            case_id = f"p2j4-dpo-v4-model-origin-v6-{family.replace('_', '-')}-{n:03d}"
            question = (
                "Use the current validated scientific state and the approved bounded action set. "
                "Choose the next action that satisfies the remaining evidence obligation, keep conclusions observational, "
                "and stop when the bounded evidence is sufficient. Do not expose raw records."
            )
            cases.append({
            ...
            n += 1
    tasks = {
        "schemaVersion": VERSION, "name": "DPO v4 model-origin collection v6",
        "purpose": "provenance_complete_model_origin_collection", "caseCount": len(cases),
        "trainingStarted": False, "reviewStatus": "SEMANTIC_PREFLIGHT_REQUIRED", "cases": cases,
    }
    audit = {
        "schemaVersion": VERSION + "-model-origin-v6-preflight", "caseCount": len(cases),
        "allNonInitialStatesSemanticNone": all(s["semanticAction"] is None for c in preflight for s in c["states"]),
        "cases": preflight,
    }
    return tasks, audit
